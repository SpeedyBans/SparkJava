...
public class SecondHighestSalary {
    public static void main(String[] s)
    {
         ...
         *-----------------------------------------------------------------------------------------------------------------------
         *  */
        Dataset<Row> DS = Spark.read().option("header",false).option("delimiter","|").schema(SalarySchemma).csv("hdfs://localhost:9000/data/input/Salary1.txt");
        ...
        Dataset<Row> DSNew = DS.withColumn("Group", functions.lit("Salary"));
         ...
         * ----------------------------------------------------------------------------------------------------------------------
         */
        WindowSpec ws = Window.partitionBy("Group").orderBy(functions.col("Salary").desc());
         ...
         * ----------------------------------------------------------------------------------------------------------------------
         */
        Dataset<Row> DSRank = DSNew.withColumn("Rank", functions.row_number().over(ws));
        ...
        Dataset<Row> finalDS = secondDS.drop("Rank").drop("Group");//we are dropping the intermediate column "Rank" added to calculate the rank per partition
