...
public class MaxSumOfSalary {
    public static void main(String[] args)
    {
        ...
        Dataset<Row> salarySum = empDept.groupBy("DepartmentID").sum("EmployeeSalary").withColumnRenamed("sum(EmployeeSalary)","SalarySum");
        ...
        Dataset<Row> salaryColumnAdd = salarySum.withColumn("DummyColumn", functions.lit(1));
        System.out.println("Printing the Schema of Dataset: salaryColumnAdd");
        salaryColumnAdd.printSchema();
        System.out.println("Printing the data of Dataset: salaryColumnAdd");
        salaryColumnAdd.show();//to show the Dataset, we do not have to run a loop on DataSet unlike RDD as this DataSet is stored more like a table and not like file

        Dataset<Row> maxSalaryDept = salaryColumnAdd.groupBy("DummyColumn").max("SalarySum","DepartmentID").withColumnRenamed("max(SalarySum)","SalarySum").withColumnRenamed("max(DepartmentID)","DepartmentID").drop("DummyColumn");
        System.out.println("Printing the Schema of Dataset: maxSalaryDept");
        maxSalaryDept.printSchema();
        System.out.println("Printing the data of Dataset: maxSalaryDept");
        maxSalaryDept.show();//to show the Dataset, we do not have to run a loop on DataSet unlike RDD as this DataSet is stored more like a table and not like file
